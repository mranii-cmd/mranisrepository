 ...
 *
 * Ajustement : conserver l'arrière-plan blanc des cases (td).
 * Les couleurs restent visibles via :
 *  - une bordure colorée à gauche de la cellule (td) lorsque toutes les séances ont le même type
 *  - pour les cellules mixtes : bordure colorée par la première couleur détectée (pas de fond)
 *  - chaque bloc de séance affiche un badge coloré et une bordure gauche colorée (fond transparent)
 *
 * Cela garantit un fond blanc uniforme pour le tableau tout en conservant un code couleur lisible.
 */
...
class TableRenderer {
    ...
    generateTableHTML(seances, hasActiveSearch = false) {
        ...
        LISTE_JOURS.forEach(jour => {
            ...
            sortedCreneaux.forEach(creneau => {
                ...
                // Déterminer style de la cellule en fonction des types présents
                let cellStyleAttr = '';
                if (seancesCell.length > 0) {
                    // récupérer les types normalisés présents dans la cellule (cours / td / tp / default)
                    const types = Array.from(new Set(seancesCell.map(s => {
                        const tn = (s.type || '').toString().trim().toLowerCase();
                        if (tn.includes('cours')) return 'cours';
                        if (tn.includes('td')) return 'td';
                        if (tn.includes('tp')) return 'tp';
                        return 'default';
                    })));

                    // Ne PAS appliquer de fond : garder arrière-plan blanc.
                    // Appliquer uniquement une bordure gauche colorée (ou la première couleur si mix).
                    const color = this.TYPE_COLORS[types[0]] || this.TYPE_COLORS.default;
                    cellStyleAttr = `style="border-left:4px solid ${color};"`;
                }

                html += `<td data-jour="${jour}" data-creneau="${creneau}" ${cellStyleAttr}
    ...
    generateSeanceHTML(seance, highlight = false) {
        ...
        // Seance bloc : garder fond transparent so background remains white; keep colored left border and badge.
        const seanceStyle = `style="background:transparent; border-left:4px solid ${color}; padding:6px 8px; margin:6px 0; border-radius:4px;"`;
        ...
        return `
        <div class="seance ${typeKey} ${highlightClass} ${nonAttribueeClass} ${sansSalleClass}" data-id="${seance.id}" ${seanceStyle}>
