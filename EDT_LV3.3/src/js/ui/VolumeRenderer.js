 ...
 *   (le forfait étant compté une seule fois car inclus dans le calcul automne uniquement).
 ...
 *
 * Ajustement : hauteur de la barre de progression principale définie à 15px.
 *
...
class VolumeRenderer {
     ...
     * la part calculée pour l'automne (le forfait étant inclus dans autumnPerTeacher).
    ...
    renderTeacherVolumes() {
        ...
        // sizes: main bar height set to 15px
        const BAR_WIDTH = 84;      // px
        const BAR_HEIGHT = 15;      // px
        const BAR_RADIUS = Math.round(BAR_HEIGHT / 2);
        ...
        teachers.forEach(teacher => {
            ...
            const progressHTML = `
                <span class="tvp-progress-wrapper" title="${totalVolume} h — ${pct}%">
                    <span class="tvp-progress-bar" style="display:inline-block; width:${BAR_WIDTH}px; height:${BAR_HEIGHT}px; background:#e9ecef; border-radius:${BAR_RADIUS}px; overflow:hidden; vertical-align:middle; position:relative;">
                    ...
                    <span class="tvp-progress-text" style="margin-left:12px; font-size:0.95rem; vertical-align:middle; min-width:48px; display:inline-block; text-align:right;">${pct}%</span>
                </span>
            `;
