            ...
            sortedCreneaux.forEach(creneau => {
                ...
                if (seancesCell.length > 0) {
                    ...
                    if (types.length === 1) {
                        ...
                        cellStyle = `style="background:${bg}; border-left:4px solid ${color};"`;
                    } else {
                        // mélange de types -> gradient entre les deux premières couleurs
                        const c1 = this.TYPE_COLORS[types[0]] || this.TYPE_COLORS.default;
                        const c2 = this.TYPE_COLORS[types[1]] || this.TYPE_COLORS.default;
                        const bg = `linear-gradient(90deg, ${this.hexToRgba(c1,0.08)} 0%, ${this.hexToRgba(c2,0.08)} 100%)`;
                        cellStyle = `style="background:${bg}; border-left:4px solid ${c1};"`;
                    }
                }
