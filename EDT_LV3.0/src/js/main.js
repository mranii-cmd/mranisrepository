   ...
   */
    ...
    populateFormSelects() {
        ...
        if (selectDepartementFiliere) {
            ...
            selectDepartementFiliere.innerHTML = '<option value="">-- Sélectionner --</option>' +
        ...
        if (selectDepartementMatiere) {
            ...
            selectDepartementMatiere.innerHTML = '<option value="">-- Sélectionner --</option>' +
                departments2.map(d => `<option value="${d}">${d}</option>`).join('');
        }
        // Matières
        ...
        const populateMatieresSelect = (filiereParam = '') => {
            ...
            selectMatiere.innerHTML = '<option value="">-- Sélectionner --</option>' +
                filtered.map(s => `<option value="${s.nom}">${s.nom}</option>`).join('');
        ...
        // const selectMatiereEl = document.getElementById('selectMatiere');
    ...
    populateTeacherSelects() {
        ...
        selects.forEach(id => {
            ...
            if (select) {
                ...
                // Construire les options en incluant data-interested et un préfixe visuel (★)
                const optionsHtml = ['<option value="">-- Aucun --</option>'].concat(
                    teachers.map(t => {
                        const isInterested = normInterested.has(t);
                        const dataAttr = isInterested ? ' data-interested="true"' : '';
                        const displayLabel = isInterested ? `★ ${t}` : t;
                        const titleAttr = isInterested && selectedMatiere ? ` title="Intéressé par ${selectedMatiere}"` : '';
                        return `<option value="${t}" data-teacher="${t}"${dataAttr}${titleAttr}>${displayLabel}</option>`;
                    })
                ).join('');

                select.innerHTML = optionsHtml;

                // Ajouter/remplacer la classe teacher-highlight si au moins un enseignant est intéressé
...
+     */
    renderTeacherWishes(enseignant) {
        ...
        // utilitaire simple d'échappement
        const esc = (s) => {
        ...
        let html = `<div class="form-section"><h4>Souhaits — ${esc(enseignant)}</h4>`;

        if (choixRows.length === 0 && !souhaits.contraintes) {
            html += `<p class="empty-message">Aucun souhait explicite pour cet enseignant.</p>`;
        } else {
            html += `<table class="wishes-preview-table" style="width:100%; border-collapse:collapse;">
                        <thead>
                            <tr>
                                <th style="text-align:left; padding:6px; border-bottom:1px solid #e9ecef;">Choix</th>
                                <th style="padding:6px; border-bottom:1px solid #e9ecef;">Cours</th>
                                <th style="padding:6px; border-bottom:1px solid #e9ecef;">TD</th>
                                <th style="padding:6px; border-bottom:1px solid #e9ecef;">TP</th>
                            </tr>
                        </thead>
                        <tbody>`;
            ...
            choixRows.forEach(row => {
                html += `<tr>
                            <td style="padding:6px; border-bottom:1px solid #f1f3f5;">${esc(row.nom)}</td>
                            <td style="padding:6px; border-bottom:1px solid #f1f3f5; text-align:center;">${esc(row.c)}</td>
                            <td style="padding:6px; border-bottom:1px solid #f1f3f5; text-align:center;">${esc(row.td)}</td>
                            <td style="padding:6px; border-bottom:1px solid #f1f3f5; text-align:center;">${esc(row.tp)}</td>
                         </tr>`;
            });
            ...
            if (souhaits.contraintes) {
                html += `<div style="margin-top:8px; color:#6c757d;"><strong>Contraintes :</strong> ${esc(souhaits.contraintes)}</div>`;
            }
        }
