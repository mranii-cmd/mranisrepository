...
class FormHandlers {
    ...
    async handleMatiereFormSubmit(event) {
        ...
        // Récupérer le département sélectionné dans le formulaire (si présent)
        // Priorité : selectDepartementMatiere (si présent dans le formulaire matière),
        // sinon fallback sur le select global #selectDepartement.
        const departement = document.getElementById('selectDepartementMatiere')?.value
            || document.getElementById('selectDepartement')?.value
            || '';
        ...
        // Déléguer la création au SubjectController (gère validation & persistence)
                const success = SubjectController.addSubject(data.nom, data);
