...
class ConfigListRenderer {
    ...
    renderGeneralSettings(containerId = 'configGeneralSettingsContainer') {
        ...
        btn.addEventListener('click', () => {
            ...
            const success = SchedulingService.setTolerance(Math.round(val));
            if (success) {
                // Mise à jour de l'affichage et feedback
                savedMsg.style.display = 'inline';
                setTimeout(() => { savedMsg.style.display = 'none'; }, 2000);
            } else {
                LogService.error('Impossible de sauvegarder la tolérance');
            }
        });
    }
