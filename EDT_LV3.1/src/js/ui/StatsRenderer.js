...
class StatsRenderer {
    ...
    renderInconsistencies() {
        ...
        subjects.forEach(subject => {
            const inconsistencies = SubjectController.checkSubjectInconsistencies(subject.nom);
        ...
        if (allInconsistencies.length === 0) {
            return `
                <div class="inconsistencies-section">
                    <h3>✅ Incohérences</h3>
                    <p class="success-message">Aucune incohérence détectée !</p>
                </div>
            `;
        }
        ...
        return `
            <div class="inconsistencies-section">
                <h3>⚠️ Incohérences Détectées (${allInconsistencies.length})</h3>
                <div class="inconsistencies-list">
                    ${allInconsistencies.map(item => `
                        <div class="inconsistency-item">
                            <div class="inconsistency-subject">${safeText(item.subject)}</div>
