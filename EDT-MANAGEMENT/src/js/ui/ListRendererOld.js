    ...
    renderTeachersList() {
        ...
        let html = `
            <div class="list-header">
                ...
                <button class="btn btn-sm btn-primary" onclick="window.EDTFormHandlers?.showAddTeacherForm?.()">
                    ➕ Ajouter
                </button>
            </div>
        ...
        if (teachers.length === 0) {
            html += '<p class="empty-message">Aucun enseignant enregistré</p>';
        } else {
            ...
            <div class="list-item teacher-item">
                <div class="item-header">
                    ...
                    <div class="item-actions">
                        <button class="btn-icon" onclick="window.EDTHandlers?.editTeacher?.('${safeText(teacher.nom)}')" title="Modifier">
                            ✏️
                        </button>
                        <button class="btn-icon btn-danger" onclick="window.EDTTeacherController?.removeTeacher('${safeText(teacher.nom)}')" title="Supprimer">
                            🗑️
                        </button>
                    </div>
                </div>
    ...
    renderSubjectsList() {
        ...
        let html = `
            <div class="list-header">
                ...
                <button class="btn btn-sm btn-primary" onclick="window.EDTFormHandlers?.showAddSubjectForm?.()">
                    ➕ Ajouter
                </button>
            </div>
            <div class="list-items">
        ...
        if (subjects.length === 0) {
            html += '<p class="empty-message">Aucune matière enregistrée</p>';
        } else {
        ...
        return `
            <div class="list-item subject-item">
                <div class="item-header">
                    ...
                    <div class="item-actions">
                        <button class="btn-icon" onclick="window.EDTSchedulingHandlers?.generateSessionsForSubject('${safeText(subject.nom)}')" title="Générer séances">
                            🚀
                        </button>
                        <button class="btn-icon" onclick="window.EDTHandlers?.editSubject?.('${safeText(subject.nom)}')" title="Modifier">
                            ✏️
                        </button>
                        <button class="btn-icon btn-danger" onclick="window.EDTSubjectController?.removeSubject('${safeText(subject.nom)}')" title="Supprimer">
                            🗑️
                        </button>
                    </div>
                </div>
    ...
    renderRoomsList() {
        ...
        let html = `
            <div class="list-header">
                ...
                <button class="btn btn-sm btn-primary" onclick="window.EDTFormHandlers?.showAddRoomForm?.()">
                    ➕ Ajouter
                </button>
            </div>
            <div class="list-items">
        ...
        if (rooms.length === 0) {
            html += '<p class="empty-message">Aucune salle enregistrée</p>';
        } else {
            rooms.forEach(room => {
        ...
        return `
            <div class="list-item room-item">
                <div class="item-header">
                    ...
                    <div class="item-actions">
                        <button class="btn-icon" onclick="window.EDTHandlers?.viewRoomSchedule?.('${safeText(room.nom)}')" title="Voir EDT">
                            📅
                        </button>
                        <button class="btn-icon btn-danger" onclick="window.EDTRoomController?.removeRoom('${safeText(room.nom)}')" title="Supprimer">
                            🗑️
                        </button>
                    </div>
                </div>
