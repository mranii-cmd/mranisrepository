    ...
    renderTeacherWishes(enseignant) {
        ...
        if (choixRows.length === 0 && !souhaits.contraintes) {
            ...
            choixRows.forEach(row => {
                html += `<tr>
                            <td style="padding:6px; border-bottom:1px solid #f1f3f5;">${escapeHTML(row.nom)}</td>
                            <td style="padding:6px; border-bottom:1px solid #f1f3f5; text-align:center;">${escapeHTML(row.c)}</td>
                            <td style="padding:6px; border-bottom:1px solid #f1f3f5; text-align:center;">${escapeHTML(row.td)}</td>
                            <td style="padding:6px; border-bottom:1px solid #f1f3f5; text-align:center;">${escapeHTML(row.tp)}</td>
                         </tr>`;
            });
            ...
            if (souhaits.contraintes) {
                html += `<div style="margin-top:8px; color:#6c757d;"><strong>Contraintes :</strong> ${escapeHTML(souhaits.contraintes)}</div>`;
            }
        }
