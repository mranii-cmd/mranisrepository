...
class VolumeRenderer {
        ...
        teachers.forEach(teacher => {
            ...
            html += `
                <tr>
                    ...
                    <td><strong>${safeText(String(totalVolume))}</strong></td>
    ...
    // --- À INSÉRER dans la classe VolumeRenderer (méthode publique) ---
...
getAnnualTeacherVolumes() {
    ...
    try {
        ...
        if (typeof this.getAnnualTeacherVolumes === 'function' && this !== undefined) {
            // évite de se rappeler récursivement (nous sommes dans la même méthode)
        }
        ...
        // 2) si on a un objet 'annualMetrics' qui contient seulement des agrégats (autumn/spring),
        //    on ne peut pas en extraire par enseignant -> on tombe sur fallback compute via VolumeService
    ...
    //    (implémentation simple: chercher le noeud interne contenant le plus de valeurs numériques)
    try {
...
// si VolumeRenderer exporte `export default new VolumeRenderer();`
// DEBUG: exposer l'instance sur window pour inspection en console (supprimez ceci en prod)
if (typeof window !== 'undefined') {
    // suppose que vous exportez `export default new VolumeRenderer();`
    // après l'export, l'instance est parfois nommée ; adaptez si nécessaire
    window.VolumeRenderer = window.VolumeRenderer || (typeof module !== 'undefined' && module.exports ? module.exports.default || module.exports : window.VolumeRenderer);
}