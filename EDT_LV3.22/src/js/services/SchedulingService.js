...
// charger séances des deux sessions sans changer l'état courant
const autumnSessionData = StorageService.loadSessionData("Session d'automne") || { seances: [] };
const springSessionData = StorageService.loadSessionData("Session de printemps") || { seances: [] };

// Forfaits par session (tentative de séparation si champ `session` présent)
...
// fallback : si annualVHM absent, retomber sur comportement sessionnel (optionnel)
if (!annualVHM) {
    try {
