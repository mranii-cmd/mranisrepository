...
class VolumeRenderer {
     ...
     * MODIFIÉ : maintenant calcule le pourcentage par enseignant :
     *   pct = 100 * (volume_enseignant / volume_reference)
     * where:
     *   - session automne : reference = (volume annuel de l'enseignant) / 2
     *   - session printemps : reference = (volume annuel de l'enseignant)
     *
     * Si la référence par enseignant n'est pas disponible, on retombe sur SchedulingService
     * ou sur StateManager.state.toleranceMaxWorkload comme fallback.
     *
     * @param {number} volumeHours
     * @param {string} teacherName
     * @returns {number} pourcentage 0-100
     */
    computeProgressPercent(volumeHours, teacherName) {
        // Try to fetch per-teacher annual volumes via getAnnualTeacherVolumes if available
        let annualMap = {};
        try {
            if (typeof this.getAnnualTeacherVolumes === 'function') {
                annualMap = this.getAnnualTeacherVolumes() || {};
            }
        } catch (e) {
            // ignore and fallback
            annualMap = {};
        ...
        // Try to read teacher-specific annual value, try several key forms
        const tryGetAnnual = (name) => {
            if (!name && name !== 0) return 0;
            const rawName = String(name || '').trim();
            if (annualMap.hasOwnProperty(rawName)) return Number(annualMap[rawName] || 0);
            // try normalized lower-case match
            const lower = rawName.toLowerCase();
            for (const k of Object.keys(annualMap || {})) {
                try {
                    if (String(k || '').trim().toLowerCase() === lower) return Number(annualMap[k] || 0);
                } catch (e) { /* ignore */ }
            }
            return 0;
        };

        const annualRaw = tryGetAnnual(teacherName);

        // Determine session header
        ...
        if (annualRaw && annualRaw > 0) {
            if (headerSession.includes('automne') || headerSession.includes('autumn')) {
                reference = Math.round(annualRaw / 2);
            } else {
                // default (printemps or other) use annual
                reference = Math.round(annualRaw);
            }
        }

        // If we still don't have a per-teacher reference, fallback to old behaviour
        if (!reference || reference <= 0) {
            ...
            if ((!reference || reference <= 0) && StateManager && StateManager.state && StateManager.state.toleranceMaxWorkload) {
                reference = Number(StateManager.state.toleranceMaxWorkload || 0);
            }
        }

        // compute percent as requested: 100 * (volume / reference)
        let pct;
        if (!reference || reference <= 0) {
            pct = volumeHours > 0 ? 100 : 0;
        } else {
            pct = Math.round((Number(volumeHours || 0) / Number(reference)) * 100);
        }
    ...
    renderTeacherVolumes() {
        ...
        teachers.forEach(teacher => {
            ...
            // compute progress percent for this teacher using teacher-specific reference
            const pct = this.computeProgressPercent(totalVolume, teacher.nom);
            ...
            const progressHTML = `
                <span class="tvp-progress-wrapper" title="${totalVolume} h — ${pct}%">
                    <span class="tvp-progress-bar" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${pct}" style="display:inline-block; width:110px; height:10px; background:#e9ecef; border-radius:6px; overflow:hidden; vertical-align:middle;">
