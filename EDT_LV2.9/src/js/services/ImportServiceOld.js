...
class ImportService {
    ...
    parseSubjectsData(data) {
        ...
        for (let i = startRow; i < data.length; i++) {
            ...
            // Default sections to 1 (prevent 0 sections resulting in 0 séances)
            const sections = (row[2] !== undefined && String(row[2]).trim() !== '') ? parseInt(row[2]) : 1;
            const tdGroups = (row[3] !== undefined && String(row[3]).trim() !== '') ? parseInt(row[3]) : 0;
            const tpGroups = (row[4] !== undefined && String(row[4]).trim() !== '') ? parseInt(row[4]) : 0;
            const volCours = (row[5] !== undefined && String(row[5]).trim() !== '') ? parseInt(row[5]) : 48;
            const volTD = (row[6] !== undefined && String(row[6]).trim() !== '') ? parseInt(row[6]) : 32;
            const volTP = (row[7] !== undefined && String(row[7]).trim() !== '') ? parseInt(row[7]) : 36;
            const nbEnsTP = (row[8] !== undefined && String(row[8]).trim() !== '') ? parseInt(row[8]) : 1;
